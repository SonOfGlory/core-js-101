...
function logger(func, logFunc) {
  return (...argums) => {
    const finalFunction = `${func.name}(${argums.map((e) => JSON.stringify(e).toString())})`;
    logFunc(`${finalFunction} starts`);
    const result = func(...argums);
    logFunc(`${finalFunction} ends`);
    return result;
  };
}
 ...
 *   partialUsingArguments(fn, 'a','b','c','d')() => 'abcd'
 */
function partialUsingArguments(fn, ...argums1) {
  return (...argums2) => fn(...argums1, ...argums2);
}
